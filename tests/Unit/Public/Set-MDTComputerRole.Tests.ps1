...
InModuleScope $ProjectName {
    ...
    Describe Set-MDTComputerRole {
        ...
        It 'Should not throw under any circumstances' {
            { Set-MDTComputerRole -Id $mocks.Id -Roles $mocks.Roles -Force } | Should -Not -Throw
        }

        It 'Should output something to the console' {
            Mock -CommandName Write-Output

            Set-MDTComputerRole -Id $mocks.Id -Roles $mocks.Roles -Force
