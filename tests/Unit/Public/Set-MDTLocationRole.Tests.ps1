...
InModuleScope $ProjectName {
    ...
    Describe Set-MDTLocationRole {
        ...
        It 'Should not throw under any circumstances' {
            { Set-MDTLocationRole -Id $mocks.Id -Roles $mocks.Roles -Force } | Should -Not -Throw
        }

        It 'Should output something to the console' {
            Mock -CommandName Write-Output

            Set-MDTLocationRole -Id $mocks.Id -Roles $mocks.Roles -Force
